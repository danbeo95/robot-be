...
class RobotSimulator {
  constructor(robotId, serverUrl = 'ws://localhost:8080') {
    ...
    // Low battery scenario configuration: Battery < 20% AND not charging
    const lowBatteryRobotId =  '00001';
    const isLowBatteryScenario = this.robotId === lowBatteryRobotId;

    // Initialize robot state with realistic values
    this.state = {
      batteryPercentage: isLowBatteryScenario ? 14 : Math.floor(Math.random() * 50) + 40,
      wifiSignalStrength: Math.floor(Math.random() * 60) - 100, // -100 to -40 dBm
      isCharging: isLowBatteryScenario ? false : Math.random() > 0.7,
  ...
  updateState() {
    const lowBatteryRobotId = process.env.LOW_BATTERY_ROBOT || '00001';

    // Battery simulation
    if (this.robotId === lowBatteryRobotId) {
      // Scenario: Keep battery < 20% AND not charging
      this.state.isCharging = false;
      this.state.batteryPercentage = Math.max(5, Math.min(18, this.state.batteryPercentage - this.batteryDrainRate * 0.05));
